...
import type { BoletoCompra, PerfilUsuario, Ruta, Viaje } from '@/entities';
...
export default async function CuentaPage() {
  ...
  const compras = comprasRes.ok
    ...
    : [];
  const viajes = viajesRes.ok ? ((await viajesRes.json()) as ViajeConRuta[]) : [];
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-zinc-600 dark:text-zinc-400">
            Consulta tu información personal, historial de compras y viajes
            disponibles.
          </p>
        </div>
        <BotonCerrarSesion />
      ...
      <section className="flex flex-col gap-4">
        ...
        {compras.length === 0 ? (
          <p className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50 px-6 py-8 text-center text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900/50 dark:text-zinc-400">
            Aún no has comprado boletos. Explora los viajes disponibles abajo.
          </p>
        ) : (
